...
describe( 'isConnectedSecondaryNetworkSite()', () => {
	it( 'should return null if no sites exist in state', () => {
		...
		expect( isConnectedSecondaryNetworkSite( state, 1 ) ).be.null;
	} );

	it( 'should return null if site with id equal to siteId is not found', () => {
		const state = {
			sites: {
				items: {
					1: {
		...
		expect( isConnectedSecondaryNetworkSite( state, 2 ) ).be.null;
	} );

	it( 'should return null if some is not yet loaded and with the loaded ones no conclusion can be taken', () => {
		const state = {
			sites: {
				items: {
					1: {},
